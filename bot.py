...
"""
بوت تلغرام متكامل + إضافات إدارة المشرفين والخصومات (كما طلبت)
- لا تغيّر أي شيء حسّاس قبل التشغيل: استبدل TOKEN/ADMIN_ID/API_KEY بما يناسبك.
- يعتمد على python-telegram-bot v13.x
"""
...
TOKEN = "8138615524:AAFr6m5Z4_gY0k7pdg7teD9nM8ReDC-KQKU"     # ← لا تستعمل توكن حقيقي علناً
...
pending_orders = []           # قائمة الطلبات المعلقة العادية
pending_cards = []            # كروت الشحن المعلقة
pending_pubg_orders = []      # شدات ببجي المعلقة
completed_orders = []         # الطلبات المكتملة (مع طابع زمني)
pending_itunes_orders = []    # طلبات ايتونز المعلقة
blocked_users = {}            # المحظورون: {user_id: True}
...
existing_col_names = [c[1] for c in existing_cols_info]
...
def add_moderator(user_id: int, full_name: str, username: str):
    ...
    cursor.execute(
        "INSERT OR REPLACE INTO moderators (user_id, full_name, username) VALUES (?, ?, ?)",
...
def remove_moderator_by_identifier(identifier: str) -> bool:
    ...
    if identifier.isdigit():
        cursor.execute("DELETE FROM moderators WHERE user_id=?", (int(identifier),))
    else:
        uname = _normalize_username(identifier)
        cursor.execute("DELETE FROM moderators WHERE LOWER(username)=LOWER(?)", (uname,))
    removed = cursor.rowcount
    conn.commit()
    return removed > 0

def list_moderators():
    cursor.execute("SELECT user_id, full_name, username FROM moderators ORDER BY user_id ASC")
    return cursor.fetchall()
...
def admin_menu_keyboard():
    buttons = [
        ...
        [InlineKeyboardButton("مراجعة الطلبات (API مكتملة)", callback_data="review_orders"),
         InlineKeyboardButton("تنفيذ الطلبات المعلقة API", callback_data="admin_pending_orders")],
...
# =========================
# النظام الجديد للإعلان (يدعم وسائط)
# =========================
...
def approve_order_process(order_index: int, context: CallbackContext, query):
    """
    يأخذ طلب من pending_orders ويحاول تنفيذه عبر API ثم ينقله إلى completed_orders
    """
    try:
    ...
    if order_info['service'] in service_api_mapping:
        ...
        if "order" in api_response:
            ...
            query.edit_message_text("تم تنفيذ الطلب عبر API وإشعار المستخدم.", reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("رجوع", callback_data="admin_pending_orders")]]))
        else:
            # فشل التنفيذ → إعادة المبلغ
            ...
            query.edit_message_text("فشل تنفيذ الطلب عبر API وتمت إعادة الرصيد للمستخدم.", reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("رجوع", callback_data="admin_pending_orders")]]))
    else:
        # خدمة غير مدموجة مع API (إكمال داخلي)
        ...
        query.edit_message_text("تم تأكيد الطلب وإشعار المستخدم.", reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("رجوع", callback_data="admin_pending_orders")]]))
...
def button_handler(update: Update, context: CallbackContext):
    ...
    # ======= قوائم المستخدم/المشرف بأسعار مخصومة للمشرف =======
    ...
    if user_id == ADMIN_ID:
        ...
        # الطلبات المكتملة عبر API (للتنبيهات/الاسترجاع)
        ...
        if data.startswith("approve_card_"):
            card_index = int(data.split("_")[-1])
            card_info = pending_cards[card_index]
            query.edit_message_text("أرسل الآن المبلغ المراد شحنه للمستخدم:", reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("رجوع", callback_data="pending_cards")]]))
        ...
        # تنفيذ الطلبات المعلقة عبر API
        if data == "admin_pending_orders":
            if not pending_orders:
                query.edit_message_text("لا توجد طلبات معلّقة حالياً.", reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("رجوع", callback_data="admin_menu")]]))
            else:
                text_msg = "الطلبات المعلقة:\n"
                kb = []
                for idx, o in enumerate(pending_orders):
                    text_msg += f"{idx+1}) {o['full_name']} | {o['service']} | {o['price']}$\n"
                    kb.append([InlineKeyboardButton(f"تنفيذ عبر API #{idx+1}", callback_data=f"approve_api_{idx}")])
...
def handle_messages(update: Update, context: CallbackContext):
    ...
    if user_id == ADMIN_ID and context.user_data.get("waiting_for_add_balance_user_id"):
        ...
        try:
            ...
            for usr in get_all_users():
                if usr[2] and usr[2].lower() == _normalize_username(target_input).lower():
                    found_user = usr
                    break
            if not found_user:
    ...
    if user_id == ADMIN_ID and context.user_data.get("waiting_for_block"):
        ident = text.strip()
        # نحوله إلى آيدي من قاعدة المستخدمين إن كان يوزر
        if ident.isdigit():
            ...
            target = None
            for usr in get_all_users():
                if usr[2] and _normalize_username(usr[2]).lower() == uname.lower():
                    target = usr[0]
                    break
            if target:
    ...
    if user_id == ADMIN_ID and context.user_data.get("waiting_for_new_mod"):
        ...
        if text.isdigit():
            ...
            row_match = None
            for usr in get_all_users():
                if usr[2] and _normalize_username(usr[2]).lower() == target_username.lower():
                    row_match = usr
                    break
            if row_match:
                target_id = row_match[0]
                full_name = row_match[1]
            else:
                update.message.reply_text("تعذر إيجاد المستخدم بهذا اليوزر في قاعدة البيانات. أرسل الآيدي الرقمي للمستخدم.")
                return

        add_moderator(target_id, full_name, target_username or "NoUsername")
        try:
        ...
        update.message.reply_text(f"تمت إضافة المشرف: {full_name} (@{target_username}) - ID: {target_id}")
        return
    ...
    # --- أوضاع المستخدم/المشرف ---
    # اختيار خدمة عامة: استقبال الرابط وخصم الرصيد
    ...
    # اختيار ببجي: استلام الآيدي وخصم
    ...
    # إرسال كود ايتونز من المالك للمستخدم
    ...
    # خدمات تلغرام: استقبال رابط الدعوة
    ...
    # شحن عبر اسياسيل: استقبال رقم الكارت
    ...
    # لا شيء من الحالات أعلاه: أظهر القائمة
