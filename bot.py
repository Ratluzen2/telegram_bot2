...
def get_effective_quantity(service_name: str) -> int:
    try:
        ...
        if ov and ov.get('quantity_multiplier') not in (None, '', 0, '0'):
...
def button_handler(update: Update, context: CallbackContext):
    ...
    if data == "show_pubg":
        ...
        for name, base_price in pubg_services.items():
            ...
            service_buttons.append([InlineKeyboardButton(format_service_label(name, eff, "pubg"), callback_data=f"pubg_service_{name}")])
        service_buttons.append([InlineKeyboardButton("رجوع", callback_data="show_services")])
